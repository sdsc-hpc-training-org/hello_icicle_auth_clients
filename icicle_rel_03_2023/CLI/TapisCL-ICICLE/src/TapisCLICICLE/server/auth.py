...
class ServerSideAuth:
    ...
    async def password_grant(self, link: str, connection):
        start = time.time()
        username_password_request = schemas.AuthRequest(request_content={"username":None, "password":None},
                                                        message={"message":"enter your TACC username and password"})
        await connection.send(username_password_request)
        username_password_response = connection.receive()
    ...
    async def auth_startup(self, connection):
        session_auth_type_request = schemas.FormRequest(request_content={"uri":None, "auth_type":None},
